...
const serviceLabel = "com.famgia.hostd"

// installCmd registers hostd as a per-user service that starts at login (and so
...
func installCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Install hostd so it runs in the background and restarts at login.

This makes hostd a long-lived daemon: combined with dtach-backed sessions, your
terminal sessions survive reboots of the daemon (and you only re-pair when a
token is revoked).`,
		RunE: func(cmd *cobra.Command, args []string) error {
			exe, err := os.Executable()
			if err != nil {
				return fmt.Errorf("locate hostd binary: %w", err)
			}
			...
			default:
				return fmt.Errorf("install is not supported on %s; run `hostd start` manually", runtime.GOOS)
			}
		},
	}
...
func uninstallCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "uninstall",
		Short: "Remove the hostd login service",
		RunE: func(cmd *cobra.Command, args []string) error {
			switch runtime.GOOS {
			case "darwin":
...
func installLaunchd(exe string, runArgs []string) error {
	...
	logPath := filepath.Join(home, "Library", "Logs", "hostd.log")
	...
	fmt.Printf("  plist : %s\n", plistPath)
	fmt.Printf("  logs  : %s\n", logPath)
	fmt.Printf("hostd now runs in the background and starts at login.\n")
	fmt.Printf("Get a pairing code anytime with: hostd pair   (or read the log)\n")
	return nil
}
...
func systemdUnitPath() (string, error) {
	...
	return filepath.Join(home, ".config", "systemd", "user", "hostd.service"), nil
}
...
func installSystemd(exe string, runArgs []string) error {
	...
	unit := "[Unit]\nDescription=hostd remote-access daemon\nAfter=network.target\n\n" +
	...
	if out, err := exec.Command("systemctl", "--user", "enable", "--now", "hostd.service").CombinedOutput(); err != nil {
		return fmt.Errorf("systemctl enable: %v: %s", err, out)
	}
	fmt.Printf("Installed systemd --user service: %s\n", unitPath)
	fmt.Printf("hostd now runs in the background. Enable lingering to start before login:\n  loginctl enable-linger %s\n", os.Getenv("USER"))
	return nil
}
...
func uninstallSystemd() error {
	...
	_ = exec.Command("systemctl", "--user", "disable", "--now", "hostd.service").Run()
	if err := os.Remove(unitPath); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove unit: %w", err)
	}
	_ = exec.Command("systemctl", "--user", "daemon-reload").Run()
	fmt.Println("Removed systemd --user service hostd.service")
	return nil
}
